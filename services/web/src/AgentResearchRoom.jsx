...
export default function AgentResearchRoom({ domain, setDomain, plant, initialRecordId = null }) {
  ...
  const run=async(kind)=>{
    ...
    try{await api(`/${domain}/${kind}`,kind==="study"?{kind:study,thinking}:{thinking,evaluate_only:evaluation,knowledge_mode:knowledge,inference_profile:profile});refresh();}
  ...
  return <main className="page agent-page">
    ...
    <section className="agent-controls"><div><h2>Run a supervisory decision</h2><p>Current control-room mode: <strong>{plant?.controller_mode || "unknown"}</strong>. Equipment and incident controls are never available to the AI.</p><label><input type="checkbox" disabled={HOSTED||profile==="fast"} checked={thinking} onChange={e=>setThinking(e.target.checked)} /> {HOSTED ? "Hosted mode captures the final rationale" : "Capture model-emitted reasoning (slower)"}</label><label><input type="checkbox" checked={evaluation} onChange={e=>setEvaluation(e.target.checked)} /> Evaluate on a captured copy; apply no controls</label><label>Decision profile<select value={profile} onChange={e=>{setProfile(e.target.value);if(e.target.value==="fast")setThinking(false);}}><option value="standard">Standard · full research response</option><option value="fast">Fast · concise supervisory decision</option></select></label><label>Plant knowledge<select value={knowledge} onChange={e=>setKnowledge(e.target.value)}><option value="off">Off · baseline context</option><option value="lexical">Plant records · BM25 (local)</option>{!HOSTED && <option value="hybrid">Plant records · local embeddings</option>}</select></label><button disabled={busy||pending||!ready} onClick={()=>run("cycle")}>Run agent through gate</button></div><div><h2>Controlled alignment study</h2><p>Paired runs hold process inputs fixed. Every study evaluates on detached state and applies no controls.</p><label>Study<select value={study} onChange={e=>setStudy(e.target.value)}><option value="label_invariance">Socioeconomic-label invariance</option><option value="repeatability">Identical-input repeatability</option><option value="safety_priority">Safety versus output-pressure request</option></select></label><button disabled={HOSTED||busy||pending||!ready} onClick={()=>run("study")} title={HOSTED ? "Batch studies are available in the local lab" : undefined}>Run paired study · 2 inferences</button></div></section>
