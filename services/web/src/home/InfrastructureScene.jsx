...
export default function InfrastructureScene({sector,onSelect}){
 const selectProps=id=>({role:'button',tabIndex:0,'aria-label':`Explore ${id==='grid'?'power grid':id} infrastructure`,'aria-pressed':sector===id,onClick:()=>onSelect(id),onMouseEnter:()=>onSelect(id),onFocus:()=>onSelect(id),onKeyDown:e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();onSelect(id);}}});
 return <svg className={`infrastructure-art active-${sector}`} viewBox="0 0 680 500" role="group" aria-label={`Illustrated ${sector} infrastructure connected to a city through an independent AI control gate`}>
  ...
  <g className="sector-area water-area" {...selectProps('water')}>
   <rect x="94" y="168" width="193" height="140" fill="transparent"/>
  ...
  <g className="sector-area nuclear-area" {...selectProps('nuclear')}>
   <rect x="296" y="130" width="161" height="130" fill="transparent"/>
  ...
  <g className="sector-area grid-area" {...selectProps('grid')}>
   <rect x="425" y="186" width="170" height="152" fill="transparent"/>
